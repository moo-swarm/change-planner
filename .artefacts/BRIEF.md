...
- [x] Action Kanban board view — 4th workspace tab (Board); 3 columns To Do/In Progress/Done; collapsible swim lanes per facet; HTML5 drag-and-drop + click-to-cycle status; extends `ActionStatus` with `'in-progress'`
...
- [x] [#50] Feature: action Kanban board view — Board tab; 3 columns (Todo/In Progress/Done); extends `ActionStatus` with `'in-progress'`; swim lanes per facet; HTML5 drag-and-drop — implemented
- [x] [#51] Feature: initiative milestone markers on roadmap timeline — `Initiative.milestones[]`; diamond markers in `RoadmapView.tsx`; reached/unreached toggle — implemented
...
### 2026-07-13 — feat: initiative milestone markers on roadmap timeline (issue #51)
- Done: added `Milestone` interface (`id/title/date/reached`) and `milestones?: Milestone[]` to `Initiative` in `types.ts`; rewrote `RoadmapView.tsx` — new `onUpdateMilestones` prop; "Add Milestone" button + inline form (title input + date picker, Enter/Escape); milestone list section showing title/date/reached-toggle/delete; merged timeline rendering via a `TimelineEntry[]` array keyed by ISO-week monday, where each entry contains both the milestone markers (rendered first as ◆ diamond dividers) and the action week block; three visual states for timeline markers: reached (grey + ✓), past+unreached (amber), future+unreached (brand violet); `App.tsx`: added `onUpdateMilestones={(milestones) => patch({ milestones })}` to the Roadmap tab's `<RoadmapView>` call; 5 new i18n keys (`roadmap.milestone/add_milestone/milestone_reached/milestone_not_reached/no_milestones`) in all 4 locales (EN/ES/BE/RU); no new npm dependencies; tsc+vite build clean (74 modules).
- Remaining: #53/#54/#55/#56/#57/#58 all past 7-day needs-review threshold; #59 (hardcoded placeholder) also past threshold (created 2026-07-05); auto-approve next in issue-number order (#53) on next research pass.
- Next task: check issues for human feedback; auto-approve #53 (cross-initiative This Week digest) if still needs-review and implement, else #54 (print-optimized view), else #55 (RACI tagging), else #56 (Vitest tests), else #57 (home screen search), else #58 (ICS export), else #59 (hardcoded placeholder 1-line fix).

### 2026-07-10 — feat: action Kanban board view (issue #50)
- Done: extended `ActionStatus` in `types.ts` to `'todo' | 'in-progress' | 'done'` (backward-compatible, legacy actions default to `'todo'`); created `BoardView.tsx` (~160 LOC) rendered as a 4th "Board" workspace tab in `App.tsx` (Workspace/Guided/Roadmap/Board/Assess) — 3 fixed columns (To Do/In Progress/Done) each with collapsible swim lanes per facet (colored dot + count + collapse toggle, state shared across columns for simplicity); action cards show text, owner, priority badge, due date (red if overdue); clicking a card cycles its status Todo → In Progress → Done → Todo (no drag needed on mobile); HTML5 drag-and-drop (draggable + onDragStart/onDragOver/onDrop) lets a card be dropped onto a different column on desktop, with a highlighted drop-target ring; empty states for the whole board and per-column. `ActionTracker.tsx`: added a clickable "In Progress" pill (amber when active) next to the priority/facet badges on each row so existing actions can be toggled into/out of in-progress without touching the add form (add form still always creates `'todo'` actions per spec); `isOverdue` broadened from `status === 'todo'` to `status !== 'done'` so in-progress overdue actions still get the red highlight. Fixed three "open action" counts that previously used `status === 'todo'` (which would have silently excluded in-progress actions) to `status !== 'done'`: `ProgressView.tsx` retro-markdown open-actions list, `HomeScreen.tsx` "Most open" sort key, and `HomeScreen.tsx` per-card open/overdue stats row. `board.tab/col_todo/col_in_progress/col_done/empty` and `actions.status_in_progress` i18n keys added to all 4 locales (EN/ES/BE/RU). No new dependencies. Verified end-to-end via Playwright against the dev server: seeded an initiative with one todo/in-progress/done action each, confirmed correct column placement and counts in both light and dark mode, confirmed click-to-cycle updates `localStorage` (todo → in-progress), and confirmed the new ActionTracker pill renders. `SharedView.tsx`'s own `status === 'todo'` open-count (read-only shared view) was intentionally left unchanged — out of the issue's stated scope (ProgressView + home screen stats only).
- Remaining: #51 (milestone markers) already auto-approved, queued next; #53/#54/#55 reached 7-day threshold 2026-07-07; #56/#57/#58 reached threshold 2026-07-10 (not yet auto-approved this run); #59 (hardcoded placeholder bug) awaiting review.
- Next task: check issues for human feedback; implement #51 (initiative milestone markers on roadmap — `Initiative.milestones[]`; diamond markers in `RoadmapView.tsx`; reached/unreached toggle; ~60 LOC), else recheck #53/#54/#55/#56/#57/#58 staleness and auto-approve first still-needs-review issue if #51 turns out already done.
